...
import { ArrowLeft, BookOpen, GraduationCap, LayoutDashboard, UsersRound, Video } from "lucide-react";
    ...
    <div className="min-h-dvh bg-[#eef3f4] lg:grid lg:grid-cols-[250px_1fr]">
      ...
      <div className="min-w-0">
        <header className="sticky top-0 z-30 border-b border-[var(--line)] bg-white/95 backdrop-blur-xl">
          <div className="container-app flex h-16 items-center justify-between"><div className="lg:hidden"><Logo compact /></div><p className="hidden font-black lg:block">Painel administrativo</p><Link href="/aluno" className="text-sm font-extrabold text-[var(--brand-dark)]">Área do aluno</Link></div>
          <nav className="container-app flex gap-1 overflow-x-auto pb-3 lg:hidden">
            {adminNav.map(({ href, label, icon: Icon }) => <Link key={href} href={href} className="flex shrink-0 items-center gap-2 rounded-xl border border-[var(--line)] bg-white px-3 py-2 text-xs font-extrabold text-[#536b73]"><Icon size={16} />{label}</Link>)}
          </nav>
        </header>
